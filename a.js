/* QUESTION 1: FULL DB + NEW PRODUCT */
const plantDB = [
    ...
    { name: "Winter Fern", price: 1100, img: "https://images.unsplash.com/photo-1512428813834-c702c7702b78?auto=format&fit=crop&w=800&q=80" }
];

let cart = [];
let qtyMap = {};

/* QUESTION 5: TOGGLE ACTION */
function toggleGallery() {
    const gallery = document.getElementById('products');
    gallery.style.display = (gallery.style.display === "none") ? "block" : "none";
}

/* QUESTION 3 & 7: VALIDATION & SAVING */
function validateAndSave() {
    const email = document.getElementById('fbEmail').value;
    const mobile = document.getElementById('fbMobile').value;
    const msg = document.getElementById('fbText').value;
    ...
    if (emailRegex.test(email) && mobileRegex.test(mobile)) {
        // Save to File
        const content = `Contact: ${email}, ${mobile}\nFeedback: ${msg}`;
        ...
        // QUESTION 7: DISPLAY RESULT
        document.getElementById('displayArea').innerText = "File Saved! Submission Code: 200 OK";
    } else {
        alert("Please provide a valid email and 10-digit mobile number.");
    }
}

/* QUESTION 4: CLEAR FORM */
function clearAndThank() {
    document.getElementById('feedbackForm').reset();
    document.getElementById('displayArea').innerText = "Form Cleared. Thank you!";
}
...
function update() {
    ...
    if (total > 1000) {
        total = total * 0.9;
        document.getElementById('total').innerText = total.toFixed(2) + " (10% Discount Applied)";
    } else {
...
// REST OF CORE FUNCTIONS RETAINED
function render() {
...
function getBtnHTML(i) {
    ...
    if (qty === 0) return `<button class="btn btn-primary" onclick="add(${i})">Add to Bag</button>`;
    return `<div class="qty-control"><button onclick="changeQty(${i}, -1)">-</button><span>${qty}</span><button onclick="changeQty(${i}, 1)">+</button></div>`;
}

function add(i) { qtyMap[i] = 1; cart.push(plantDB[i]); update(); document.getElementById(`btn-area-${i}`).innerHTML = getBtnHTML(i); }

function changeQty(i, delta) {
    if (delta === 1) { qtyMap[i]++; cart.push(plantDB[i]); } 
    else { 
        qtyMap[i]--; 
...
function checkout() { alert("Order Confirmed!"); cart = []; qtyMap = {}; update(); render(); closeModal('cartModal'); }
